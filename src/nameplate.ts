 ...
 * whole and only `quillfeather` and `brightwater-sync` are cut. At eight, seven
...
export function choose(text: string, wpx: number, hpx: number) {
	...
	for (const font of LADDER) {
		...
		// of that bought nothing visible and cost `foxglove` its last letter on a
		// small terminal font.
	...
	// Bounded to one step above the floor: `willow` can physically take 5x where
	// `brookwater` takes 3x, and that much variation stops looking like a size and
