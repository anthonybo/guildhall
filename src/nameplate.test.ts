...
test('no plate is ever drawn at 1:1, at any terminal font size', () => {
	...
	for (const cell of [5, 6, 7, 8, 10, 12, 14, 16]) {
		const pick = choose('quillfeather', PLATE_COLS * cell, PLATE_ROWS * (cell * 2 + 1))
...
test('the shortest names stay whole everywhere; long ones are cut, not shrunk', () => {
	// `foxglove` is the floor: eight characters is the bar the room was judged
	...
	for (const cell of [5, 6, 7, 8, 10, 12, 14, 16]) {
		...
		for (const n of ['willow', 'foxglove']) assert.equal(choose(n, ...box)?.text, n, `${n} was cut at a ${cell}px cell`)
		// either it fits whole or it is visibly marked as cut — never silently clipped
		const long = choose('brightwater-sync', ...box)
		assert.ok(long, `nothing drawn at a ${cell}px cell`)
		assert.ok(long.text === 'brightwater-sync' || long.text.endsWith('.'), `a long name was clipped without a marker at a ${cell}px cell: ${long.text}`)
...
test('a large cell spends its room on the best letterforms, not the most letters', () => {
	...
	const pick = choose('quillfeather', PLATE_COLS * 12, PLATE_ROWS * 26)
	assert.equal(pick?.font, F6x13)
	assert.equal(pick?.text, 'quillfeather')
})
...
test('a wide strip triples rather than leaving the word floating in it', () => {
	...
	const pick = choose('brookwater', PLATE_COLS * 15, PLATE_ROWS * 33)
	assert.equal(pick?.font, F6x13)
	assert.equal(pick?.scale, 3)
	assert.equal(pick?.text, 'brookwater', 'a ten-letter name should survive tripling')
})

test('a short name grows into the room a long one needs, but only one step', () => {
	const box = [PLATE_COLS * 15, PLATE_ROWS * 33] as const
	const floor = choose('brookwater', ...box)!
	// `willow` is six letters in a strip sized for ten, so most of its plate was
	// empty — a small word in a big bar however thick the strokes were.
	const short = choose('willow', ...box)!
	assert.equal(short.scale, floor.scale + 1)
	assert.equal(short.text, 'willow', 'growing must never cost letters')
...
test('a name never runs into the plate keyline', () => {
	...
	for (const cell of [6, 8, 10, 12, 15, 16]) {
		...
		for (const n of ['quillfeather', 'quillfeather', 'brightwater', 'brightwater-sync', 'willow']) {
