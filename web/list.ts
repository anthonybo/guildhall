...
const cardOf = (state: string) =>
  mix(LOOK[state].color, WEIGHT[state] ?? 0.1, PANEL);
const bandOf = (state: string) =>
  mix(LOOK[state].color, WEIGHT[state] ?? 0.1, BG);
...
function details(s: Session) {
  ...
  dl.className =
    "[grid-area:detail] mt-2.5 grid [grid-template-columns:max-content_1fr] gap-x-3.5 gap-y-1 border-t border-line pt-2.5 text-[0.82rem]";
  const rows: [string, string][] = [
    ...
    [
      "context",
      s.ctxUsed
        ? `${tokens(s.ctxUsed)} of ${tokens(s.ctxLimit)}`
        : "nothing yet",
    ],
    ["idle", ago(s.stale)],
    ...(s.tab ? ([["tab", `⌘${s.tab}`]] as [string, string][]) : []),
    ...(s.waitingFor
      ? ([["waiting on", s.waitingFor]] as [string, string][])
      : []),
    ...(s.last && s.last !== s.doing
      ? ([["last said", s.last]] as [string, string][])
      : []),
  ];
